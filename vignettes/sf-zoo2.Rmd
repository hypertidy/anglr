---
title: "Simple features and the R spatial zoo"
...
These are the nodes in the graph, we need to traverse the edges to determine the arc-branches, and anything left is just a standalone arc.  This is brute-force remove duplicate edges (ignore direction), join on start vertex, end vertex, count the number of edges involved and any with 3 edges is a **Node**. 
...
These hierarchical structures can be serialized and stored in different ways, typically they are stored as binary geoms and stored directly in a table. As the Spatial Data topic says under "Why Geoms and not More Normal Form?" this is very performant, since a small number of large records can be optimized better compared to a large number of small records. 
