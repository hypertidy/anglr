...
nodes <- mp$v %>% dplyr::select(vertex_) %>% inner_join(mp$bXv) %>% distinct(vertex_, branch_) %>% group_by(vertex_) %>% mutate(nb = n()) %>%  ungroup() %>% filter(nb > 2) %>% distinct(vertex_) %>% inner_join(mp$v)
plot(x1)
