...
As well as the data strucutre models, anglr includes the following visualization functions. 
...
The density can be controlled by setting `max_area` (other controls are available via arguments to [RTriangle::triangulate]). Note that we now have new vertices that did not exist in the original polygon layer, both interal to the surface of the polygon and along its boundary. This is the "mesh-quality" aspect mentioned above. 
